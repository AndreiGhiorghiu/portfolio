...
`);
  ...
  cli.command(
  ...
  ).option("--silent", "Don't print any logs").option("-m, --minify", "Minify generated code").option("--clean", "Clean the chunks before generating").option("-c, --config <path>", "Path to panda config file").option("-w, --watch", "Watch files and rebuild").option("--minimal", "Do not include CSS generation for theme tokens, preflight, keyframes, static and global css").option("-p, --poll", "Use polling instead of filesystem events when watching").option("-o, --outfile [file]", "Output file for extracted css, default to './styled-system/styles.css'").option("--cwd <cwd>", "Current working directory", { default: cwd }).action(async (maybeGlob, flags = {}) => {
    ...
    const cssgen = async (ctx2) => {
      if (outfile) {
        ...
        if (cssArtifact) {
          const { msg } = await (0, import_node.generateCssArtifactOfType)(ctx2, cssArtifact, outfile);
          import_logger.logger.info("css:emit:artifact", msg);
          return;
        }
        if (minimal) {
          const { msg } = await (0, import_node.bundleMinimalFilesCss)(ctx2, outPath);
          import_logger.logger.info("css:emit:min", msg);
        } else {
        ...
        if (cssArtifact) {
          import_logger.logger.warn("cssgen:warn", "Cannot generate css artifact without an outfile");
        }
        const { msg } = await (0, import_node.writeAndBundleCssChunks)(ctx2);
        import_logger.logger.info("css:emit:bundle", msg);
      }
    };
    await cssgen(ctx);
    if (watch) {
