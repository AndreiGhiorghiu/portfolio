  ...
  cli.command(
    ...
    const cssgen = async (ctx2) => {
      if (outfile) {
        ...
        if (cssArtifact) {
          const { msg } = await generateCssArtifactOfType(ctx2, cssArtifact, outfile);
          logger.info("css:emit:artifact", msg);
          return;
        }
        if (minimal) {
          const { msg } = await bundleMinimalFilesCss(ctx2, outPath);
        ...
        const { msg } = await writeAndBundleCssChunks(ctx2);
